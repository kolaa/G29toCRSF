...
void tuh_hid_umount_cb(uint8_t dev_addr, uint8_t instance)
{
  ...
  if (gamepad_mounted && ds4_dev_addr == dev_addr && ds4_instance == instance)
  {
    ...
    gamepad_state.brake = USHRT_MAX;
    gamepad_state.ruddle = USHRT_MAX/2;
  }
}
...
static inline uint16_t read_uint16_sensor(uint8_t const* report, uint16_t pos)
{
  uint16_t sensor = report[pos+1] << 8;
  sensor |= report[pos];
  return sensor;
}
...
void process_report(uint8_t const* report, uint16_t len)
{
  ...
  static sony_ds4_report_t prev_ds4_report = { 0 };
  ...
  if (report_id == 1)
  {
    ...
    prev_ds4_report.counter = ds4_report.counter;
    ...
    if ( diff_report(&prev_ds4_report, &ds4_report) )
    {
      gamepad_state.x = ds4_report.x;
      gamepad_state.y = ds4_report.y;
      gamepad_state.z = ds4_report.z;
      gamepad_state.rz = ds4_report.rz;
      gamepad_state.l2 = ds4_report.l2;
      gamepad_state.l3 = ds4_report.l3;
      gamepad_state.r2 = ds4_report.r2;
      gamepad_state.r3 = ds4_report.r3;
    ...
    if (len>62) 
    {
        gamepad_state.ruddle = read_uint16_sensor(report, 42);
        gamepad_state.throttle = WHEEL_SENSOR_MAX - read_uint16_sensor(report, 44);
        gamepad_state.brake = read_uint16_sensor(report, 46);

        printf("wheel r:%u  t:%u  b:%u\r\n", gamepad_state.ruddle, gamepad_state.throttle, gamepad_state.brake);
    }    

    prev_ds4_report = ds4_report;
  }
}
